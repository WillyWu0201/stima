...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => {
      // Let scrollable inner elements (e.g. an iPhone screen with
      // overflow:auto content) handle the wheel themselves, instead of
      // hijacking every wheel for pan/zoom.
      if (!(e.ctrlKey || e.metaKey)) {
        ...
        while (el && el !== vp && el.nodeType === 1) {
          ...
          if ((oy === 'auto' || oy === 'scroll') && el.scrollHeight > el.clientHeight) {
            const dy = e.deltaY;
            const canScrollDown = el.scrollTop + el.clientHeight < el.scrollHeight - 0.5;
            const canScrollUp = el.scrollTop > 0.5;
            if ((dy > 0 && canScrollDown) || (dy < 0 && canScrollUp)) {
              // browser will scroll the element — don't preventDefault
              return;
            }
          }
          el = el.parentElement;
